    ...
    <div style="margin: auto;">
      <table class="table table-striped">
        ...
        <tr v-for="item in pagingSummary.data">
          ...
          <td class="failure" v-if="item.assertions.failed > 0" v-on:click="showFailures(collectionId, distribute, item.assertions.failures, 'Assertions Failures')">{{item.assertions.failed}}</td>
          <td v-else>0</td>
          <td>{{item.testScripts.success}}</td>
          <td class="failure" v-if="item.testScripts.failed > 0" v-on:click="showFailures(collectionId, distribute, item.testScripts.failures, 'TestScripts Failures')">{{item.testScripts.failed}}</td>
          <td v-else>0</td>
        </tr>
        </tbody>
      </table>
...
<script>
  ...
  export default {
    ...
    props: ['collectionId', 'summary', 'distribute'],
    data() {
      return {
    ...
    methods: {
      ...
      update(summary) {
        ...
        this.failLineDataCollection = {
          ...
          datasets: [
            {
              label: this.distribute + ' assertions failures count',
            ...
            {
              label: this.distribute + ' testScripts failures count',
